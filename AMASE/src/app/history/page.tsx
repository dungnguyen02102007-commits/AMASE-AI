  ...
  return (
    <div className="bg-white rounded-2xl border border-black/[0.07] shadow-sm p-12 text-center">
      ...
      <p className="text-[13px] text-slate-500 mt-1.5 max-w-sm mx-auto">
        Upload a resume on the dashboard and it&apos;ll appear here so you can revisit past analyses.
      </p>
      <Link
        href="/dashboard"
        className="inline-block mt-5 text-[13px] font-semibold text-white bg-slate-900 hover:bg-black rounded-xl px-4 py-2 transition-colors"
      >
        Analyze a resume →
      </Link>
    </div>
    ...
    >
      ...
      >
        ...
        <p className="text-[13px] text-slate-500 mt-1.5">
          This will delete {count} saved {count === 1 ? "analysis" : "analyses"} from your browser. This can&apos;t be undone.
        </p>
        <div className="flex gap-2 mt-5">
          <button
            onClick={onCancel}
            className="flex-1 py-2.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-700 text-[13px] font-semibold transition-colors"
          >
            Cancel
          </button>
          <button
        ...
        <div className="flex items-end justify-between gap-4 mb-6 flex-wrap">
          <div>
            ...
            <p className="text-[13px] text-slate-500 mt-1">
              {entries.length === 0
                ? "No analyses yet"
                : `${entries.length} saved · stored in your browser`}
            </p>
          </div>
          ...
          {entries.length > 0 && (
            <button
              ...
              className="text-[12px] font-semibold text-slate-600 border border-slate-200 rounded-lg px-3 py-1.5 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors"
            >
              Clear all
            </button>
          )}
